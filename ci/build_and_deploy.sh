...
if [ "${IN_CI}" == "FALSE" ]; then
  ...
  ssh -i ~/.ssh/ski-for-web.pem jngo@4dvd.sdsu.edu "mkdir -p /var/www/html/${COPY_PATH}"
  scp -i ~/.ssh/ski-for-web.pem -rp ./dist/* jngo@4dvd.sdsu.edu:/var/www/html/${COPY_PATH}
else
